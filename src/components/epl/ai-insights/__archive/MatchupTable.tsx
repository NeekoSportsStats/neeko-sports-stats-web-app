...
import type { MatchupRow, PremiumMode } from "./types";
import { cx } from "./utils";

function badge(label: MatchupRow["label"]) {
  if (label === "Advantage") return "border-amber-400/35 bg-amber-500/15 text-amber-100";
  if (label === "Disadvantage") return "border-white/20 bg-white/5 text-white/70";
  return "border-white/15 bg-white/5 text-white/75";
}

export default function MatchupTable(props: { rows: MatchupRow[]; mode: PremiumMode }) {
  const locked = props.mode !== "premium";

  return (
    <div className="grid gap-3">
      <div className="overflow-hidden rounded-2xl border border-white/10">
        <div className="grid grid-cols-[1.4fr_0.7fr_0.7fr_1.6fr] bg-white/5 px-3 py-2 text-[11px] uppercase tracking-wide text-white/55">
          <div>Matchup</div>
          <div>Label</div>
          <div>Delta</div>
          <div>AI Insight</div>
        ...
        <div className="divide-y divide-white/10">
          {props.rows.map((r) => (
            <div key={r.key} className="grid grid-cols-[1.4fr_0.7fr_0.7fr_1.6fr] px-3 py-3">
              <div className="min-w-0">
                <div className="truncate text-sm font-medium text-white">{r.title}</div>
                <div className="mt-0.5 text-xs text-white/55">
                  Reliability: {Math.round(r.reliability01 * 100)}%
                </div>
              </div>
