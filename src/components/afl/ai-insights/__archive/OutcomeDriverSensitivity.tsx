...
import type { OutcomeDriver, PremiumMode } from "./types";
...
export function OutcomeDriverSensitivity(props: { drivers: OutcomeDriver[]; mode: PremiumMode }) {
          ...
          <div key={d.key} className="rounded-2xl border border-white/10 bg-white/5 p-3">
            ...
            <div className="relative mt-2">
              {locked ? (
                ...
                <div className="line-clamp-2 text-sm text-white/80">{d.aiSummary}</div>
              )}
            </div>
          </div>
        ))}
