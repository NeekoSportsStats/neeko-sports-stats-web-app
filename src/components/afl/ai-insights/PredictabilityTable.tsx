    ...
    <>
              ...
              <div key={team}>
                ...
                <div className="px-6 py-2 bg-white/5 border-y border-white/10">
                  ...
                  <div className="mt-1 text-[11px] text-white/40">
                    Top 5 players · {FREE_PER_TEAM} free
                  </div>
                </div>

                {/* ROWS */}
                {filtered.map((r, i) => {
                  const rowLocked = locked && i >= FREE_PER_TEAM;

                  return (
                    <div
                      key={r.id}
                      onClick={() => {
                        if (!rowLocked) setActiveRow(r);
                      }}
                      className={cx(
      ...
      {activeRow && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
          <div className="w-[420px] rounded-2xl border border-white/10 bg-gradient-to-b from-slate-900 to-black p-5">
            ...
            <div className="mt-4 rounded-lg bg-white/5 p-3">
              <div className="text-sm font-semibold text-white">
                {activeRow.rangeLow} → {activeRow.rangeHigh}
              </div>
              <div className="mt-2 h-2 rounded bg-white/10">
                <div className="h-2 w-full rounded bg-amber-400/80" />
              </div>
            ...
            <div className="mt-3 flex gap-2 text-xs">
              <span className="rounded-full border border-white/10 px-2 py-1 text-white/60">
                Confidence: {confLabel(activeRow.confidence01)}
              </span>
              <span className="rounded-full border border-white/10 px-2 py-1 text-white/60">
                Volatility: {volLabel(activeRow.volatility01)}
              </span>
            </div>
          </div>
        </div>
