...
/* -------------------------------------------------------------------------------------------------
  STEP 2A–E — HERO SCATTER POLISH (LOCKED)
-------------------------------------------------------------------------------------------------- */
...
export default function PlayerImpactScatterPanel(props: {
  ...
  const visible = players.filter(
    (p) => teamFilter === "both" || p.side === teamFilter
  );
    ...
    <section className="rounded-3xl border border-amber-400/20 bg-gradient-to-b from-[#0c0c0c] to-black p-5">
      ...
      <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-black/40">
        <svg viewBox={`0 0 ${W} ${H}`} className="w-full">
          ...
          {/* QUADRANT LABELS */}
          <text x={PAD + 8} y={PAD + 18} fill={selected && selected.momentum < 50 && selected.ceiling >= 50 ? "#fbbf24" : "rgba(255,255,255,0.4)"} fontSize={12}>
            Volatile upside
          </text>
          <text x={W - PAD - 110} y={PAD + 18} fill={selected && selected.momentum >= 50 && selected.ceiling >= 50 ? "#fbbf24" : "rgba(255,255,255,0.4)"} fontSize={12}>
            Finale targets
          </text>
          <text x={PAD + 8} y={H - PAD - 8} fill={selected && selected.momentum < 50 && selected.ceiling < 50 ? "#fbbf24" : "rgba(255,255,255,0.4)"} fontSize={12}>
            Low impact
          </text>
          <text x={W - PAD - 110} y={H - PAD - 8} fill={selected && selected.momentum >= 50 && selected.ceiling < 50 ? "#fbbf24" : "rgba(255,255,255,0.4)"} fontSize={12}>
            Safe, capped
          </text>
          ...
          {visible.map((p) => {
            ...
            return (
              <g key={p.id}>
                {isSelected && (
                  <>
                    <line x1={cx} y1={PAD} x2={cx} y2={H - PAD} stroke="#fbbf24" opacity={0.35} strokeDasharray="4 4" />
                    <line x1={PAD} y1={cy} x2={W - PAD} y2={cy} stroke="#fbbf24" opacity={0.35} strokeDasharray="4 4" />
                  </>
                )}
      ...
      {selected && (
        <div className="mt-4 rounded-xl border border-amber-400/30 bg-amber-400/10 px-4 py-3">
          ...
          <div className="mt-2 flex gap-4 text-sm">
            <div>Momentum: <b>{selected.momentum}</b></div>
            <div>Ceiling: <b>{selected.ceiling}</b></div>
          </div>
          <div className="mt-2 text-xs text-white/60">
            Click to explore weekly trend, projection band & risk
          </div>
        </div>
      )}
    </section>
