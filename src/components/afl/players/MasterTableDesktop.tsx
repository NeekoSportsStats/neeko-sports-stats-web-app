// src/components/afl/players/MasterTableDesktop.tsx
...
const SPACING = {
  ...
  statRowGrid: "grid grid-cols-[28px_1fr] gap-2",
  hitRateGapY: "space-y-1",
  dividerColor: "bg-yellow-500/10",
  col3Grid: "grid-cols-[112px_1px_1fr]",
};
...
/** Deterministic mock values that change with stat lens */
function getRowValues(key: string, stat: StatLens): number[] {
  let seed = 0;
  for (let i = 0; i < key.length; i++) seed += key.charCodeAt(i);

  const base =
    stat === "Fantasy" ? 70 : stat === "Disposals" ? 18 : 0;

  const spread =
    stat === "Fantasy" ? 40 : stat === "Disposals" ? 22 : 4;

  return ROUND_LABELS.map((_, i) =>
    Math.round(base + ((seed + i * 11) % spread))
  );
}
    ...
    <>
      <div className="mt-10 rounded-3xl border border-neutral-800 bg-black/90 shadow-2xl overflow-hidden">
        ...
        <div className="px-6 py-6 border-b border-neutral-800 space-y-4">
          <div className="flex items-start justify-between">
            ...
            <div className="flex gap-2 rounded-full border border-neutral-700 bg-black/80 p-1">
              {(["Fantasy", "Disposals", "Goals"] as StatLens[]).map(
                (s) => (
                  <button
                    key={s}
        ...
        <div className="relative overflow-x-auto scrollbar-none">
          ...
          >
            ...
            >
              <div className="sticky top-0 z-10 px-4 py-3 bg-black/95 border-b border-neutral-800
                  ...
                  <div className={cx("grid h-full items-center", SPACING.col3Grid)}>
                    ...
                    <div className={cx("flex flex-col justify-center", SPACING.statsGapY)}>
                      {[
                        ...
                        <div key={l as string} className={SPACING.statRowGrid}>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md">
          <div className="relative w-full max-w-md rounded-3xl border border-yellow-500/40
                          bg-gradient-to-b from-neutral-950 via-black to-black px-6 py-6">
            <button
            ...
            <h3 className="mt-2 text-xl font-semibold text-neutral-50">
              Unlock full player master grid
            </h3>
            <p className="mt-2 text-sm text-neutral-300">
              Search, team filters, full round breakdowns and premium insights.
            </p>

            <div className="mt-6 flex gap-3">
              <a
                href="/neeko-plus"
                className="flex-1 rounded-2xl bg-yellow-400 py-2 text-center
              ...
              <button
                onClick={() => setShowModal(false)}
                className="flex-1 rounded-2xl border border-neutral-700 py-2 text-neutral-200"
              >
                Maybe later
              </button>
            </div>
