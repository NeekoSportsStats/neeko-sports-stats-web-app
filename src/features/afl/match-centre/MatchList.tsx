...
import type { DayGroup, MatchSummary } from "@/lib/afl/matchCenter";

function formatTime(localIso: string | null | undefined, utcIso: string | null | undefined) {
    ...
    <div className="space-y-8">
      {groups.map((g, idx) => (
        <div key={idx} className="space-y-4">
          <div className="text-white/80 font-semibold">{g.day_label}</div>
          ...
          <div className="space-y-4">
            {g.matches.map((m) => {
              ...
              return (
                <button
                  key={m.vendor_game_id ?? m.match_id ?? idx}
                  onClick={() => {
                ...
                >
                  ...
                  <div className="mt-4 flex flex-wrap items-center gap-4 text-sm text-white/60">
                    <div>{venue}</div>
                    <div>{formatTime(m.game_time_local, m.game_time)}</div>
