...
const HEADER_BASE = "h-9 flex items-center text-[10px] font-semibold uppercase tracking-wider text-white/35 select-none whitespace-nowrap";
const PREMIUM_COLS = ["price", "value", "aiRec", "why"] as const;

function isPremiumCol(col: (typeof PREMIUM_COLS)[number] | string): boolean {
...
function HeaderFixed() {
  return (
    <div className="flex shrink-0" style={{ width: COL.rank + COL.player + COL.rating }}>
...
function HeaderScroll({ isPremium }: { isPremium: boolean }) {
  const cols: { key: string; label: string; premium: boolean; width: number }[] = [
  ...
  return (
    <div className="flex">
      {cols.map(({ key, label, premium, width }) => (
        <div
          key={key}
...
function FixedCells({ row, idx, tier, isPremium, onTap }: Pick<RowProps, "row" | "idx" | "tier" | "isPremium" | "onTap">) {
  const rank = idx + 1;
  ...
  return (
    <div
      className="flex shrink-0 cursor-pointer active:bg-white/[0.05] transition-colors"
      style={{ width: COL.rank + COL.player + COL.rating }}
      onClick={onTap}
    >
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-1 min-w-0">
            ...
            {!isPremium && isUnlocked && (
              <span className="shrink-0 rounded-sm bg-[#F5C84C]/15 px-1 py-px text-[8px] font-semibold text-[#F5C84C] uppercase">Free</span>
            )}
          </div>
...
function ScrollCells({ row, idx, tier, isPremium, activeTab, onUpgrade, onTap }: RowProps) {
  ...
  return (
    <div className="flex cursor-pointer active:bg-white/[0.03] transition-colors" onClick={onTap}>
      ...
      <div className={`${CELL_BASE} justify-center`} style={{ width: COL.price, minWidth: COL.price }}>
        {locked("price") ? <LockedPlaceholder /> : (
      ...
      <div className={`${CELL_BASE} justify-center flex-col gap-0.5`} style={{ width: COL.value, minWidth: COL.value }}>
        {locked("value") ? <LockedPlaceholder /> : (
          <>
      ...
      <div className={`${CELL_BASE} justify-center px-2`} style={{ width: COL.aiRec, minWidth: COL.aiRec }}>
        {locked("aiRec") ? <LockedPlaceholder /> : displayRec ? (
          <span
...
function BlurredRow({ idx, onUpgrade }: { idx: number; onUpgrade: () => void }) {
  return (
    ...
    >
      <div
        className="flex shrink-0 opacity-25 blur-[3px] select-none pointer-events-none"
        style={{ width: COL.rank + COL.player + COL.rating }}
      >
...
// ─── Conversion wall (inline, not full-page) ───────────────────────────────────
...
export function MobileRankingsTable({
  ...
  const FREE_PARTIAL_ROWS_LOCAL = FREE_PARTIAL_ROWS;
  ...
  return (
    <div>
      ...
      <div className="rounded-xl border border-white/5 overflow-hidden">
        {/* Header */}
        <div className="flex bg-[#0a0a0a] border-b border-[#222] sticky top-0 z-20">
          <div className="flex shrink-0 sticky left-0 z-30 bg-[#0a0a0a]" style={{ width: COL.rank + COL.player + COL.rating }}>
            <HeaderFixed />
          </div>
          <div
          ...
          >
            <HeaderScroll isPremium={isPremium} />
          </div>
        </div>

        {/* Body — two-pane scroll */}
        <div
          ...
          {loading ? (
            ...
            return visibleRows.map((row, idx) => {
              const tier: RowTier = isPremium ? "premium" : (
                idx < FREE_FULL_ROWS ? "full" : idx < FREE_PARTIAL_ROWS_LOCAL ? "partial" : "locked"
              );

              if (!isPremium && idx >= FREE_PARTIAL_ROWS_LOCAL) {
              ...
              return (
                ...
                >
                  <div
                    className="sticky left-0 z-10 bg-[#070707] shrink-0"
                    style={{ width: COL.rank + COL.player + COL.rating }}
                  >
                    <FixedCells
                      row={row}
