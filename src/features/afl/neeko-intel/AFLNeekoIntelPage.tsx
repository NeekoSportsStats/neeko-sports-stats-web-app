      ...
      <div className="bg-gradient-to-r from-[#3A2A00] via-[#5A4200] to-[#3A2A00] border border-[#F5C84C]/30 p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div>
          ...
          <p className="text-white/60 text-sm">
            Access all breakouts, risers, captain picks and AI reasoning for every player.
          </p>
        </div>
        <a
          href="/neeko-plus"
...
// ─── Generic Player Cards ─────────────────────────────────────────────────────

function renderPlayerCards(rows: PlayerRow[], loading: boolean, error: boolean) {
  if (loading) {
...
export default function AFLNeekoIntelPage() {
  ...
  const breakouts = allData
    .filter((p) => (p.upside_rating ?? 0) >= 8 && (p.risk_rating ?? 100) <= 40)
    .slice(0, 10);

  const risk = allData
    .filter((p) => (p.risk_rating ?? 0) >= 65 || (p.projection_confidence ?? 100) <= 40)
    .slice(0, 10);

  const risers = allData
    .filter((p) => (p.form_rating ?? 0) >= 70 && (p.projection_confidence ?? 0) >= 60)
    .slice(0, 10);

  const fallers = allData
    .filter((p) => (p.projection_confidence ?? 100) <= 40 && (p.risk_rating ?? 0) >= 50)
    .slice(0, 10);
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        ...
        {!fetchError && (
          <>
            ...
            <EliteCaptainHero rows={captains} loading={loading} error={fetchError} />
            ...
            <Section>
              ...
              />
              <div className="space-y-3">
                {renderPlayerCards(visibleBreakouts, loading, fetchError)}
              </div>
            </Section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Section>
                ...
                />
                <div className="space-y-3">
                  {renderPlayerCards(visibleRisers, loading, fetchError)}
                </div>
              </Section>
              ...
              <Section>
                ...
                />
                <div className="space-y-3">
                  {renderPlayerCards(visibleFallers, loading, fetchError)}
                </div>
              </Section>
            </div>
            ...
            <Section>
              ...
              />
              <div className="space-y-3">
                {renderPlayerCards(visibleRisk, loading, fetchError)}
              </div>
            </Section>
